...
class HomecareWithMotion(MycroftSkill):
    def __init__(self):
        ...
        self.ask_to_inform = ["cannot walk", "can't walk", "can't wake up", "cannot wake up"]
    ...
    def handle_motion(self, message):
        ...
        now = now_local()
        gap = timedelta(seconds=65)  # random value
        ...
        if len(immediate_help_words) > 0:
            ...
            if yes_no == "yes":
                ...
                if no_email_flag:
                    ...
                    self.speak("ok, just ask me if you need to contact")
            else:
                title = ask_to_inform_words[0]
                self.mail(title, response)
